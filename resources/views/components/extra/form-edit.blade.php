...
<div class="grid grid-cols-12">
    <form action="#" method="post" id="form-add" data-object="{{@$object}}" class="col-span-12">
        <div class="mt-5 grid grid-cols-12 gap-6">
            @for($i=0;$i<sizeof($inputs);$i++)
                <div class="intro-y col-span-{{$card_sizes[$i]}}">
                    ...
                    <div class="preview-component intro-y box">
                        <div class="flex flex-col items-center border-b border-slate-200/60 p-5 dark:border-darkmode-400 sm:flex-row">
                            ...
                            <div data-tw-merge="" class="flex items-center mt-3 w-full sm:ml-auto sm:mt-0 sm:w-auto"><label data-tw-merge="" for="inputs-group-{{$i}}-btn-min" class="cursor-pointer ml-2">min</label>
                                <input data-tw-merge="" data-wrap-id="inputs-group-{{$i}}" type="checkbox" class="minmax {{$style['checkbox']}}" id="inputs-group-{{$i}}-btn-min">
                            </div>
                        </div>
                        <div class="p-5" id="inputs-group-{{$i}}">
                            <div class="preview relative [&.hide]:overflow-hidden [&.hide]:h-0">
                            @foreach ($inputs[$i]['elements'] as $input)
                                <div class="form-group mt-3">
                                    ...
                                    @switch($input['type'])
                                        @case('text')
                                            <input  class="{{@$input['class']}} {{$style['input']}}"
                                        ...
                                        @break
                                        @case('textarea')
                                            <textarea   class="{{@$input['class']}} {{$style['input']}}"
                                                        name="{{$input['var_name']}}" placeholder="{{@$input['format']}}"
                                                        {{@$input['is_required']?'required':''}} style="{{@$input['height']?'height:'.@$input['height']:''}}" rows="5">{{ isset($input['is_eppgbm']) && $input['is_eppgbm'] ? $selected[$input['var_name']] : $selected[$input['var_name']] }}</textarea>
                                        @break
                                        @case('editor')
                                        ...
                                        @case('select')
                                            ...
                                            @endphp --}}
                                            <select class="{{@$input['is_hidden']?'hidden':''}} {{@$input['class']}} {{$style['select']}}" 
                                                    name="{{$input['var_name']}}" 
                                                    ...
                                                    @foreach ($input['select_attr']['options'] as $item)
                                                        <option value="{{$item[$input['select_attr']['id']]}}"
                                                            ...
                                                            {{$item[$input['select_attr']['label']]}}
                                                             <!-- || {{$item[$input['select_attr']['id']]}} || {{$selected[$input['var_name']]}} -->
                                                        </option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        ...
                                        @case('radio')
                                            ...
                                            <div class="form-check">
                                                    <input  class="form-check-input" type="radio" id="{{$input['var_name']}}" 
                                                            name="{{$input['var_name']}}" value="{{$item[$input['select_attr']['id']]}}"
    ...
    <form action="#" method="post" id="form-add-custom" data-object="{{@$object}}" class="col-span-12">
        @if(@$custom_inputs)
                                                        ...
                                                        <div class="mt-5 hidden" id="accordion-collapse-menu-position" aria-labelledby="accordion-collapse-heading-position">
                                                            ...
                                                            <table class="mx-auto w-full my-5">
                                                                <tbody>
                                                                    ...
                                                                    <tr>
                                                                        <td>
                                                                            <select class="{{$style['select']}}" id="position-draft-type" style="width:100px" readonly>
                                                                        ...
                                                                        <td>
                                                                            <select class="{{$style['select']}}" id="position-draft-position" style="width:100px">
    ...
    <button type="button" id="btn-submit-add"
            class="fixed bottom-0 right-0 z-50 mb-5 mr-5 flex py-2 px-3 cursor-pointer items-center justify-center rounded-md bg-theme-1 text-white text-sm shadow-lg">
        Simpan Baru
    </button>
</div>